...
import de.gnwi.jdpdsp.samples.logger.MemoryLogger;
...
import de.gnwi.jdpdsp.tests.interactions.ParticleIndexPairCalculatorForTesting;
import de.gnwi.jdpdsp.tests.interactions.ParticleIndexPairNumberCalculatorForTesting;
import de.gnwi.jdpdsp.tests.interactions.ParticleIndexPairRandomCalculatorForTesting;
...
public class ParticlePairInteractionCalculatorTest extends TestCase {
    ...
    public void test_IndexPairCount_ParallelSize_Performance() {
        ...
        String tmpOffset = "> ";
        for (float tmpBoxLength = 20.0f; tmpBoxLength <= 200.0f; tmpBoxLength += 10.0f) {
            // NOTE for tests:
            ...
            float tmpCutOffLength = 1.0f;
            // float tmpBoxLength = 100.0f;
            ...
            MemoryLogger tmpMemoryLogger = new MemoryLogger(new int[] {ILogLevel.EXCEPTION});
            ParticleArrays tmpParticles = new ParticleArrays(
                new float[tmpParticleNumber],
                new float[tmpParticleNumber],
                new float[tmpParticleNumber],
                null, null, null, null, 
            ...
            for (int i = 0; i < tmpParticleNumber; i++) {
                tmpParticles.getR_x()[i] = tmpBoxLength * tmpRandomDpd.nextFloat();
                tmpParticles.getR_y()[i] = tmpBoxLength * tmpRandomDpd.nextFloat();
                tmpParticles.getR_z()[i] = tmpBoxLength * tmpRandomDpd.nextFloat();
            }

            // new ParticleIndexPairNumberCalculator -------------------------------

            tmpStart = System.currentTimeMillis();
            ParticleIndexPairNumberCalculatorForTesting tmpParticleIndexPairNumberCalculator = new ParticleIndexPairNumberCalculatorForTesting(new Factory(Factory.RandomType.ACRNG_MT_64, 1, Factory.DpdType.CUTOFF_LENGTH_ONE, Factory.ElectrostaticsType.AD_HOC, Factory.BondType.HARMONIC, Factory.IntegrationType.GWMVV, new Float[] {0.65f}), tmpMemoryLogger, tmpBoxSize, tmpPeriodicBoundaries, tmpCutOffLength, null, null, new ParallelizationInfo(2, 2, tmpParallelTaskNumber), tmpRandomNumberSeed);
            tmpEnd = System.currentTimeMillis();
            System.out.println(tmpOffset + "new ParticleIndexPairNumberCalculator,                  t [s] = " + String.valueOf((tmpEnd - tmpStart)/1000.0f));
            ...
            tmpEnd = System.currentTimeMillis();
            System.out.println(tmpOffset + "calculateCellBasedParticlePairInteractions(parallel),   t [s] = " + String.valueOf((tmpEnd - tmpStart)/1000.0f));
            ...
            System.out.println(tmpOffset + "Time in nanoseconds per particle index pair,           t [ns] = " + String.valueOf((int) ((float) (tmpEnd - tmpStart)/(float) tmpNumber1 * 1000000.0f)));
    ...
    public void test_IndexPairCount_ParallelProcessor_Performance() {
        ...
        for (int tmpParallelTaskNumber = 16; tmpParallelTaskNumber >= 1; tmpParallelTaskNumber--) {
            ...
            ParticleIndexPairNumberCalculatorForTesting tmpParticleIndexPairNumberCalculator = new ParticleIndexPairNumberCalculatorForTesting(new Factory(Factory.RandomType.ACRNG_MT_64, 1, Factory.DpdType.CUTOFF_LENGTH_ONE, Factory.ElectrostaticsType.AD_HOC, Factory.BondType.HARMONIC, Factory.IntegrationType.GWMVV, new Float[] {0.65f}), tmpMemoryLogger, tmpBoxSize, tmpPeriodicBoundaries, tmpCutOffLength, null, null, new ParallelizationInfo(2, 2, tmpParallelTaskNumber), tmpRandomNumberSeed);
            tmpEnd = System.currentTimeMillis();
            System.out.println(tmpOffset + "new tmpParticleIndexPairNumberCalculator,             t [s] = " + String.valueOf((tmpEnd - tmpStart)/1000.0f));
    ...
    private void test_IndexPairGeneration(int aParallelTaskNumber) {
        ...
        tmpEnd = System.currentTimeMillis();
        System.out.println("new ParticleIndexPairCalculator,            t [s] = " + String.valueOf((tmpEnd - tmpStart)/1000.0f));
        ...
        tmpEnd = System.currentTimeMillis();
        System.out.println("calculateCellBasedParticlePairInteractions, t [s] = " + String.valueOf((tmpEnd - tmpStart)/1000.0f));
        ...
        tmpEnd = System.currentTimeMillis();
        System.out.println("calculateLoopBasedParticlePairInteractions, t [s] = " + String.valueOf((tmpEnd - tmpStart)/1000.0f));
    ...
    private void test_ParticlePairDistanceParametersCache(int aParallelTaskNumber) {
        ...
        tmpEnd = System.currentTimeMillis();
        System.out.println("calculateCacheBasedParticlePairInteractions, t [s] = " + String.valueOf((tmpEnd - tmpStart)/1000.0f));
        tmpParticleIndexPairStrings2 = this.convertIntegerIndexPairQueue(tmpParameters.getTestObjects().getParticleIndexPairQueue());
    ...
    private void test_IndexPairRandomGeneration(int aParallelTaskNumber) {
        ...
        AtomicInteger tmpRandomNumberSeed = new AtomicInteger(1);
        tmpStart = System.currentTimeMillis();
        ParticleIndexPairRandomCalculatorForTesting tmpParticleIndexPairRandomCalculator = new ParticleIndexPairRandomCalculatorForTesting(new Factory(Factory.RandomType.ACRNG_MT_64, 1, Factory.DpdType.CUTOFF_LENGTH_ONE, Factory.ElectrostaticsType.AD_HOC, Factory.BondType.HARMONIC, Factory.IntegrationType.GWMVV, new Float[] {0.65f}), tmpMemoryLogger, tmpBoxSize, tmpPeriodicBoundaries, tmpCutOffLength, null, null, new ParallelizationInfo(2, 2, aParallelTaskNumber), tmpRandomNumberSeed);
        tmpEnd = System.currentTimeMillis();
        System.out.println("new tmpParticleIndexPairRandomCalculator,   t [s] = " + String.valueOf((tmpEnd - tmpStart)/1000.0f));
    ...
    private void test_IndexPairCount(int aParallelTaskNumber) {
        ...
        ParticleIndexPairNumberCalculatorForTesting tmpParticleIndexPairNumberCalculator = new ParticleIndexPairNumberCalculatorForTesting(new Factory(Factory.RandomType.ACRNG_MT_64, 1, Factory.DpdType.CUTOFF_LENGTH_ONE, Factory.ElectrostaticsType.AD_HOC, Factory.BondType.HARMONIC, Factory.IntegrationType.GWMVV, new Float[] {0.65f}), tmpMemoryLogger, tmpBoxSize, tmpPeriodicBoundaries, tmpCutOffLength, null, null, new ParallelizationInfo(2, 2, aParallelTaskNumber), tmpRandomNumberSeed);
        tmpEnd = System.currentTimeMillis();
        System.out.println("new ParticleIndexPairNumberCalculator,      t [s] = " + String.valueOf((tmpEnd - tmpStart)/1000.0f));
