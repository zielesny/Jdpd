...
import de.gnwi.jdpd.interfaces.IInput;
...
public class FileInput implements IInput {
    ...
    public Factory getFactory() {
        ...
        if (tmpValues[0].equals(Factory.IntegrationType.GWMVV.toString())) {
            ...
            // Double parameter: Lambda parameter for Groot-Warren Modified Velocity-Verlet (GWMVV) integration
            tmpIntegrationParameters = 
                new Object[] {
                    Double.valueOf(tmpValues[1])
                };
            ...
            // Double parameter: Mu parameter for nonsymmetric pairwise Nose–Hoover–Langevin thermostat (PNHLN) integration
    ...
    public ParticleTypes getParticleTypes() {
        ...
        double[] aMolarMasses = new double[tmpParticleTable.length];
        double[] aCharges = new double[tmpParticleTable.length];
        for (int i = 0; i < tmpParticleTable.length; i++) {
            ...
            aCharges[i] = Double.valueOf(tmpParticleTable[i][1]);
            aMolarMasses[i] = Double.valueOf(tmpParticleTable[i][2]);
        }
    ...
    public MoleculeDescription[] getMoleculeDescriptions() {
        ...
        for (int i = 0; i < tmpMoleculeTable.length; i++) {
            ...
            double[] tmpR_x = new double[tmpTotalMoleculeParticleNumber];
            double[] tmpR_y = new double[tmpTotalMoleculeParticleNumber];
            double[] tmpR_z = new double[tmpTotalMoleculeParticleNumber];
            ...
            for (int j = 0; j < tmpTotalMoleculeParticleNumber; j++) {
                ...
                tmpR_x[j] = Double.valueOf(tmpPositionsBondsFileContent[j + 2][3]);
                tmpR_y[j] = Double.valueOf(tmpPositionsBondsFileContent[j + 2][4]);
                tmpR_z[j] = Double.valueOf(tmpPositionsBondsFileContent[j + 2][5]);
                // Read bonds
    ...
    public BoxSize getBoxSize() {
        double[] tmpBoxSizeParameters =  this.getTripleDoubleValues(Section.CHEMICAL_SYSTEM_DESCRIPTION, FileInput.BOX_SIZE);
    ...
    public MoleculeBoundaryDescription[] getMoleculeBoundaryDescriptions() {
        ...
        for (int i = 0; i < tmpMoleculeBoundaryTable.length; i++) {
            ...
            if ((tmpIsActiveX || tmpIsActiveY || tmpIsActiveZ) && tmpMaxTimeStep > 0) {
                ...
                double tmpYmax = Double.valueOf(tmpMoleculeBoundaryTable[i][6]);
                double tmpZmin = Double.valueOf(tmpMoleculeBoundaryTable[i][8]);
                double tmpZmax = Double.valueOf(tmpMoleculeBoundaryTable[i][9]);
    ...
    public MoleculeVelocityFixationDescription[] getMoleculeVelocityFixationDescriptions() {
        ...
        for (int i = 0; i < tmpMoleculeVelocityFixationTable.length; i++) {
            ...
            double tmpVelocityX = Double.valueOf(tmpMoleculeVelocityFixationTable[i][2]);
    ...
    public MoleculeAccelerationDescription[] getMoleculeAccelerationDescriptions() {
        ...
        for (int i = 0; i < tmpMoleculeAccelerationTable.length; i++) {
            ...
            double tmpAccelerationX = Double.valueOf(tmpMoleculeAccelerationTable[i][1]);
    ...
    public double getNearestNeighborDistance() {
        return this.getSingleDoubleValue(Section.CHEMICAL_SYSTEM_DESCRIPTION, FileInput.NEAREST_NEIGHBOR_DISTANCE);
    }
    // </editor-fold>
    ...
    public double getTemperature() {
        return this.getSingleDoubleValue(Section.INTERACTION_DESCRIPTION, FileInput.TEMPERATURE);
    }
    ...
    public double getDpdSigma() {
        return this.getSingleDoubleValue(Section.INTERACTION_DESCRIPTION, FileInput.DPD_SIGMA);
    }
     ...
     */
    @Override
    public double[][] getAij(ParticleTypes aParticleTypes) {
        String[][] tmpInteractionTable = this.getValueTable(Section.INTERACTION_DESCRIPTION, FileInput.INTERACTION_TABLE);
        // Quadratic matrix
        double[][] tmpAij = new double[aParticleTypes.getParticleTypeNumber()][aParticleTypes.getParticleTypeNumber()];
        for (String[] tmpInteractionTableLine : tmpInteractionTable) {
            ...
            double tmpAijValue = Double.valueOf(tmpInteractionTableLine[2]);
    ...
    public GravitationalAcceleration getGravitationalAcceleration() {
        double[] tmpGraviationalAccelerationParameters =  this.getTripleDoubleValues(Section.INTERACTION_DESCRIPTION, FileInput.GRAVITATIONAL_ACCELERATION);
    ...
    public double getTimeStepLength() {
        return this.getSingleDoubleValue(Section.SIMULATION_DESCRIPTION, FileInput.TIME_STEP_LENGTH);
    }
    ...
    /**
     * Returns single double value of parameter
     * 
     * @param aSection Section tag
     * @param aParameterString Parameter string
     * @return Single double value of parameter
     * @throws IllegalArgumentException Thrown if an argument is illegal.
     */
    private double getSingleDoubleValue(Section aSection, String aParameterString) throws IllegalArgumentException {
        // <editor-fold defaultstate="collapsed" desc="Checks">
        if (aSection == null || !this.sectionMap.containsKey(aSection)) {
            throw new IllegalArgumentException("FileInput.getSingleDoubleValue: aSection is null/empty or does not exist.");
        }
        if (aParameterString == null || aParameterString.isEmpty()) {
            throw new IllegalArgumentException("FileInput.getSingleDoubleValue: aParameterString is null/empty.");
        }
        // </editor-fold>
        try {
            String[][] tmpSection = this.sectionMap.get(aSection);
            for (String[] tmpSectionLine : tmpSection) {
                if (tmpSectionLine[0].equalsIgnoreCase(aParameterString)) {
                    if (tmpSectionLine.length != 2) {
                        throw new IllegalArgumentException("FileInput.getSingleDoubleValue: tmpSectionLine has wrong format.");
                    }
                    return Double.valueOf(tmpSectionLine[1]);
                }
            }
            throw new IllegalArgumentException("FileInput.getSingleDoubleValue: aParameterString could not be found in section.");
        } catch (Exception anException) {
            throw new IllegalArgumentException("FileInput.getSingleDoubleValue: aParameterString can not be evaluated.", anException);
        }
    }
    
    /**
     * Returns triple double value of parameter
     * 
     * @param aSection Section tag
     * @param aParameterString Parameter string
     * @return Triple double value of parameter
     * @throws IllegalArgumentException Thrown if an argument is illegal.
     */
    private double[] getTripleDoubleValues(Section aSection, String aParameterString) throws IllegalArgumentException {
        // <editor-fold defaultstate="collapsed" desc="Checks">
        if (aSection == null || !this.sectionMap.containsKey(aSection)) {
            throw new IllegalArgumentException("FileInput.getTripleDoubleValues: aSection is null/empty or does not exist.");
        }
        if (aParameterString == null || aParameterString.isEmpty()) {
            throw new IllegalArgumentException("FileInput.getTripleDoubleValues: aParameterString is null/empty.");
        }
        // </editor-fold>
        try {
            String[][] tmpSection = this.sectionMap.get(aSection);
            for (String[] tmpSectionLine : tmpSection) {
                if (tmpSectionLine[0].equalsIgnoreCase(aParameterString)) {
                    if (tmpSectionLine.length != 4) {
                        throw new IllegalArgumentException("FileInput.getTripleDoubleValues: tmpSectionLine has wrong format.");
                    }
                    return new double[] {Double.valueOf(tmpSectionLine[1]), Double.valueOf(tmpSectionLine[2]), Double.valueOf(tmpSectionLine[3])};
                }
            }
            throw new IllegalArgumentException("FileInput.getTripleDoubleValues: aParameterString could not be found in section.");
        } catch (Exception anException) {
            throw new IllegalArgumentException("FileInput.getTripleDoubleValues: aParameterString can not be evaluated.", anException);
        }
    }
    ...
    /**
     * Returns sixtupel double value of parameter
     * 
     * @param aSection Section tag
     * @param aParameterString Parameter string
     * @return Sixtupel double  value of parameter
     * @throws IllegalArgumentException Thrown if an argument is illegal.
     */
    private double[] getSixtupelDoubleValues(Section aSection, String aParameterString) throws IllegalArgumentException {
        // <editor-fold defaultstate="collapsed" desc="Checks">
        if (aSection == null || !this.sectionMap.containsKey(aSection)) {
            throw new IllegalArgumentException("FileInput.getSixtupelDoubleValues: aSection is null/empty or does not exist.");
        }
        if (aParameterString == null || aParameterString.isEmpty()) {
            throw new IllegalArgumentException("FileInput.getSixtupelDoubleValues: aParameterString is null/empty.");
        }
        // </editor-fold>
        try {
            String[][] tmpSection = this.sectionMap.get(aSection);
            for (String[] tmpSectionLine : tmpSection) {
                if (tmpSectionLine[0].equalsIgnoreCase(aParameterString)) {
                    if (tmpSectionLine.length != 7) {
                        throw new IllegalArgumentException("FileInput.getSixtupelDoubleValues: tmpSectionLine has wrong format.");
                    }
                    return new double[] {
                        Double.valueOf(tmpSectionLine[1]), 
                        Double.valueOf(tmpSectionLine[2]), 
                        Double.valueOf(tmpSectionLine[3]),
                        Double.valueOf(tmpSectionLine[4]), 
                        Double.valueOf(tmpSectionLine[5]), 
                        Double.valueOf(tmpSectionLine[6])
                    };
                }
            }
            throw new IllegalArgumentException("FileInput.getSixtupelDoubleValues: aParameterString could not be found in section.");
        } catch (Exception anException) {
            throw new IllegalArgumentException("FileInput.getSixtupelDoubleValues: aParameterString can not be evaluated.", anException);
        }
    }
