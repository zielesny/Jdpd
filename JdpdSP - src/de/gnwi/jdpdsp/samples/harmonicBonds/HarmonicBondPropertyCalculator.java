 ...
 * thread-safe objects and accumulation items like non thread-safe float adders 
 * and standard (NON atomic) float arrays.
 * 
...
public abstract class HarmonicBondPropertyCalculator implements IHarmonicBondPropertyCalculator {
    ...
    private class PartialBondChunkCalculationTask implements Callable<Boolean> {
        ...
        public Boolean call() {
            ...
            float[] tmpBondLengths = this.parallelizationSafeBondChunkArrays.getBondLengths();
            float[] tmpForceConstants = this.parallelizationSafeBondChunkArrays.getForceConstants();
        ...
        try {
            ...
            for (HarmonicBondChunkArrays tmpBondChunkArrays : aBondChunkArraysList) {
                ...
                if (this.parallelizationInfo.getParallelTaskNumber() > 1 && tmpCurrentParallelTaskNumber > 1) {
                    ...
                    float[] tmpBondLengths = tmpBondChunkArrays.getBondLengths();
                    float[] tmpForceConstants = tmpBondChunkArrays.getForceConstants();
