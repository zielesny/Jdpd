...
public class ParticlePairElectrostaticsAdHocPotentialCalculator extends ParticlePairInteractionCalculator {
 ...
 (see ParticlePairInteractionCalculator for details)
    ...
    protected void calculateParticlePairInteraction(
        ...
        final float tmpEffectiveChargeProduct = tmpParticleCharge1 * tmpElectrostatics.getEffectiveChargeFactor() * tmpParticleCharge2 * tmpElectrostatics.getEffectiveChargeFactor();
        // Potential energy
        ...
        if (tmpElectrostatics.getEffectiveExponent() == 2.0) {
            ...
            tmpFactor = tmpEffectiveChargeProduct / (float) FastMath.pow(tmpRij, tmpElectrostatics.getEffectiveExponent());
        }
        ...
        if (tmpFactor > tmpElectrostatics.getMaximumAbsoluteForceValue() || tmpFactor < -tmpElectrostatics.getMaximumAbsoluteForceValue()) {
            ...
            float tmpRijFmax;
            if (tmpElectrostatics.getEffectiveExponent() == 2.0) {
                tmpRijFmax = (float) FastMath.sqrt(FastMath.abs(tmpEffectiveChargeProduct)/tmpElectrostatics.getMaximumAbsoluteForceValue());
                tmpPotentialEnergy = tmpEffectiveChargeProduct / tmpRijFmax;
            } else {
                tmpRijFmax = (float) FastMath.pow(FastMath.abs(tmpEffectiveChargeProduct)/tmpElectrostatics.getMaximumAbsoluteForceValue(), 1.0f / tmpElectrostatics.getEffectiveExponent());
                tmpPotentialEnergy = tmpEffectiveChargeProduct / (float) FastMath.pow(tmpRijFmax, tmpElectrostatics.getEffectiveExponent() - 1.0);
            }
            if (tmpRij <= tmpElectrostatics.getDampingDistance()) {
            ...
            if (tmpElectrostatics.getEffectiveExponent() == 2.0) {
                ...
                tmpForceTerm = tmpEffectiveChargeProduct / (float) FastMath.pow(tmpRijFmax, tmpElectrostatics.getEffectiveExponent() + 1.0);
            }
            if (tmpRij <= tmpElectrostatics.getDampingDistance()) {
            ...
            // </editor-fold>
        } else if (tmpFactor != 0.0f) {
            // <editor-fold defaultstate="collapsed" desc="|F| <= Fmax and |F| != 0">
            if (tmpElectrostatics.getEffectiveExponent() == 2.0) {
                tmpPotentialEnergy = tmpEffectiveChargeProduct / tmpRij;
            } else {
                tmpPotentialEnergy = tmpEffectiveChargeProduct / (float) FastMath.pow(tmpRij, tmpElectrostatics.getEffectiveExponent() - 1.0);
            }
            if (tmpRij <= tmpElectrostatics.getDampingDistance()) {
                tmpPotentialEnergy *= tmpElectrostatics.getDampingFactor();
            }
            ...
            if (tmpElectrostatics.getEffectiveExponent() == 2.0) {
                ...
                tmpForceTerm = tmpEffectiveChargeProduct / (float) FastMath.pow(tmpRij, tmpElectrostatics.getEffectiveExponent() + 1.0);
            }
            if (tmpRij <= tmpElectrostatics.getDampingDistance()) {
                tmpForceTerm *= tmpElectrostatics.getDampingFactor();
            }
