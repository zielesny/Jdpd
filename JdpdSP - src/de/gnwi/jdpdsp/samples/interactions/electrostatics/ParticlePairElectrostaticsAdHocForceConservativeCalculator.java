 ...
 */
public class ParticlePairElectrostaticsAdHocForceConservativeCalculator extends ParticlePairInteractionCalculator implements IParticlePairForceCalculator {
     ...
     */
    public ParticlePairElectrostaticsAdHocForceConservativeCalculator(
        Factory aFactory,
    ...
    protected void calculateParticlePairInteraction(
        ...
        if (tmpElectrostatics.getEffectiveExponent() == 2.0) {
            ...
            tmpFactor = tmpParticleCharge1 * tmpElectrostatics.getEffectiveChargeFactor() * tmpParticleCharge2 * tmpElectrostatics.getEffectiveChargeFactor() / (float) FastMath.pow(tmpRij, tmpElectrostatics.getEffectiveExponent());
        }
        if (tmpRij <= tmpElectrostatics.getDampingDistance()) {
            tmpFactor *= tmpElectrostatics.getDampingFactor();
        }
        if (tmpFactor > tmpElectrostatics.getMaximumAbsoluteForceValue()) {
            tmpFactor = tmpElectrostatics.getMaximumAbsoluteForceValue();
        } else if (tmpFactor < -tmpElectrostatics.getMaximumAbsoluteForceValue()) {
            tmpFactor = -tmpElectrostatics.getMaximumAbsoluteForceValue();
        }
        // Electrostatics force
        final float tmpFij_x = tmpFactor * aRij_x / tmpRij;
        final float tmpFij_y = tmpFactor * aRij_y / tmpRij;
        final float tmpFij_z = tmpFactor * aRij_z / tmpRij;
    ...
    protected void calculateParticlePairInteraction(
        ...
        // Method not defined
        throw new IllegalStateException("ParticlePairElectrostaticsAdHocForceConservativeCalculator.calculateParticlePairInteraction: Overload NOT supported.");
    }
    // </editor-fold>
