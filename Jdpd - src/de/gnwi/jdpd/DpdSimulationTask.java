...
import de.gnwi.jdpd.samples.harmonicBonds.HarmonicBondChunkGenerator;
...
import de.gnwi.jdpd.parameters.SimulationCounts;
 ...
 *
 */
public class DpdSimulationTask implements Callable<Boolean>, IDpdSimulationTask {
        ...
        try {
            ...
            for (int i = 0; i < tmpTimeStepNumber; i++) {
                ...
                if (tmpCurrentTimeStep == 1 || tmpCurrentTimeStep%this.parameters.getSimulationDescription().getTimeStepFrequencyForOutput() == 0) {
                    ...
                    // Surface tension
                    double tmpPressureX = tmpTimeStepCalculator.getPotentialAccumulator().getExtendedAdderGroup().getPressureXAdder().getSum();
                    double tmpPressureY = tmpTimeStepCalculator.getPotentialAccumulator().getExtendedAdderGroup().getPressureYAdder().getSum();
                    double tmpPressureZ = tmpTimeStepCalculator.getPotentialAccumulator().getExtendedAdderGroup().getPressureZAdder().getSum();
                    double tmpDpdPressureX = tmpTimeStepCalculator.getPotentialAccumulator().getExtendedAdderGroup().getDpdPressureXAdder().getSum();
                    double tmpDpdPressureY = tmpTimeStepCalculator.getPotentialAccumulator().getExtendedAdderGroup().getDpdPressureYAdder().getSum();
                    double tmpDpdPressureZ = tmpTimeStepCalculator.getPotentialAccumulator().getExtendedAdderGroup().getDpdPressureZAdder().getSum();
                    double tmpBoxXLength = this.parameters.getChemicalSystemDescription().getBoxSize().getXLength();
                    double tmpBoxYLength = this.parameters.getChemicalSystemDescription().getBoxSize().getYLength();
                    double tmpBoxZLength = this.parameters.getChemicalSystemDescription().getBoxSize().getZLength();
                    double tmpSurfaceTensionAlongZ = (tmpPressureZ - 0.5 * (tmpPressureX + tmpPressureY)) / (tmpBoxXLength * tmpBoxYLength);
                    ...
                    // Utotal
                    double tmpUtotal = tmpUpotTotal + tmpUkin;
                    // Temperature
                    double tmpTemperature = tmpUkin / (ONE_A_HALF * (double) this.parameters.getParticleArrays().getV_x().length);
                    ...
                    if (this.parameters.getChemicalSystemDescription().isRgCalculation()) {
                        ...
                        for (int k = 0; k < tmpRgCalculators.length; k++) {
                            double tmpMeanRadiusOfGyration = tmpRgCalculators[k].getMeanRadiusOfGyration(
                                this.parameters.getParticleArrays().getR_x(),
                                this.parameters.getParticleArrays().getR_y(),
                                this.parameters.getParticleArrays().getR_z(),
                ...
                double tmpSimulationProgressInPercent = ((double) i + 1.0) / (double) tmpTimeStepNumber * HUNDRED;
                ...
                long tmpTotalTimeMillis = (long) (HUNDRED / tmpSimulationProgressInPercent * (double) tmpProgressTimeMillis);
    ...
    private void setParameters(IInput aSimulationInput, RestartInfo aRestartInfo) {
        ...
        double[] tmpR_x = new double[tmpSimulationCounts.getParticleNumber()];
        double[] tmpR_y = new double[tmpSimulationCounts.getParticleNumber()];
        double[] tmpR_z = new double[tmpSimulationCounts.getParticleNumber()];
        ...
        double[] tmpParticleCharges = new double[tmpSimulationCounts.getParticleNumber()];
        double[] tmpParticleDpdMasses = new double[tmpSimulationCounts.getParticleNumber()];
        double[] tmpParticleMolarMasses = new double[tmpSimulationCounts.getParticleNumber()];
        ...
        for (int i = 0; i < tmpMoleculeDescriptions.length; i++) {
            ...
            for (int j = 0; j < tmpMoleculeDescription.getTotalMoleculeParticleNumber(); j++) {
                ...
                if (tmpSimulationDescription.isDpdUnitMass()) {
                    tmpParticleDpdMasses[tmpCurrentIndex] = 1.0;
                } else {
                ...
                if (tmpSingleParticleBondOffsets != null) {
                    for (int k = 0; k < tmpSingleParticleBondOffsets.length; k++) {
                        // Only backward bonds are evaluated to avoid double counting
        ...
        double[] tmpMoleculeMolarMasses = new double[tmpMoleculeNames.length];
        for (int i = 0; i < tmpMoleculeNames.length; i++) {
            double tmpMoleculeMolarMass = 0;
        ...
        double[] tmpNearestNeighborDistances = null;
        ...
        if (tmpNearestNeighborParticleDescriptions != null) {
            ...
            tmpNearestNeighborDistances = new double[tmpSimulationCounts.getParticleNumber()];
        ...
        if (aRestartInfo == null) {
            tmpParticleArrays = 
                new ParticleArrays(
                    ...
                    tmpR_y,
                    tmpR_z,
                    new double[tmpSimulationCounts.getParticleNumber()],
                    new double[tmpSimulationCounts.getParticleNumber()],
                    new double[tmpSimulationCounts.getParticleNumber()],
                    tmpParticleTokens,
    ...
    private void initialiseVelocities() {
        ...
        // </editor-fold>
        double tmpVelocityScaleFactor = Utils.scale_v(
            this.parameters.getParticleArrays().getV_x(),
            this.parameters.getParticleArrays().getV_y(),
            this.parameters.getParticleArrays().getV_z(),
            this.parameters.getParticleArrays().getDpdMasses(),
        ...
        if (this.parameters.getChemicalSystemDescription().getMoleculeBoundaryInfos() != null) {
            ...
            for (MoleculeBoundaryInfo tmpMoleculeBoundaryInfo: this.parameters.getChemicalSystemDescription().getMoleculeBoundaryInfos()) {
                if (tmpMoleculeBoundaryInfo.isBoundaryX()) {
                    double tmpXmin = tmpMoleculeBoundaryInfo.getXmin();
                    double tmpXmax = tmpMoleculeBoundaryInfo.getXmax();
                ...
                if (tmpMoleculeBoundaryInfo.isBoundaryY()) {
                    double tmpYmin = tmpMoleculeBoundaryInfo.getYmin();
                    double tmpYmax = tmpMoleculeBoundaryInfo.getYmax();
                ...
                if (tmpMoleculeBoundaryInfo.isBoundaryZ()) {
                    double tmpZmin = tmpMoleculeBoundaryInfo.getZmin();
                    double tmpZmax = tmpMoleculeBoundaryInfo.getZmax();
